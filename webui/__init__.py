...
def index():
    if request.method == 'POST':
        if request.form.get('Valid') == 'Valid':
            ...
            db.session.add(data)
            db.session.commit()
        elif request.form.get('Invalid') == 'Invalid1':
            data = Dataset.query.filter_by(file_path =  session['file_path']).first()
            data.instance_validated= 1
            data.file_with_user = 0
            data.instance_valid= -1
            ...
            db.session.add(data)
            db.session.commit()
        elif request.form.get('Invalid') == 'Invalid2':
            data = Dataset.query.filter_by(file_path =  session['file_path']).first()
            data.instance_validated= 1
            data.file_with_user = 0
            data.instance_valid= -2
            data.user_validated = session['username']
            db.session.add(data)
            db.session.commit()
        elif request.form.get('Invalid') == 'Invalid3':
            data = Dataset.query.filter_by(file_path =  session['file_path']).first()
            data.instance_validated= 1
            data.file_with_user = 0
            data.instance_valid= -3
            data.user_validated = session['username']
            db.session.add(data)
            db.session.commit()
        elif request.form.get('Invalid') == 'Invalid4':
            data = Dataset.query.filter_by(file_path =  session['file_path']).first()
            data.instance_validated= 1
            data.file_with_user = 0
            data.instance_valid= -4
            data.user_validated = session['username']
            db.session.add(data)
            db.session.commit()
        elif request.form.get('Invalid') == 'Invalid5':
            data = Dataset.query.filter_by(file_path =  session['file_path']).first()
            data.instance_validated= 1
            data.file_with_user = 0
            data.instance_valid= -5
            data.user_validated = session['username']
            db.session.add(data)
            db.session.commit()
